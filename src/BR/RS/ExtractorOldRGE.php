...
class ExtractorOldRGE extends Extractor
{
    ...
    {
        if (str_contains($value, "0601 Energia Ativa Fornecida - TE")) {
            $lineRaw = preg_replace('/\s+/', ' ', substr($value, 41));
            $lineExplode = explode(' ', $lineRaw);
            $this->bill["Taxes"]["TE"] = [];
    ...
    {
        if (str_contains($value, "Contrib. Custeio IP")) {
            $lineRaw = preg_replace('/\s+/', ' ', substr($value, 40));
            $lineExplode = explode(' ', $lineRaw);
            $this->bill["Taxes"]["IP"] = [];
